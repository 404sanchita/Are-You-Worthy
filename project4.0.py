...
def draw_ground():
    ...
    # Grid lines (raised slightly to avoid z-fighting)
    glColor3f(0.2, 0.5, 0.2)
    glBegin(GL_LINES)
    for i in range(-int(ENV_SIZE/2), int(ENV_SIZE/2)+1, 2):
        glVertex3f(i, 0.05, -ENV_SIZE)   # was 0.01 → changed to 0.05
...
def select_nearest_tile():
    ...
    # Find the nearest uncollected tile
    nearest_tile = None
    ...
    # If a tile is close enough, select it
    if nearest_tile and min_distance < 2.0:
        ...
        # Replace the collected tile with a new random letter
        new_letter = random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZ')
        nearest_tile["letter"] = new_letter
        nearest_tile["collected"] = False  # Make it collectible again
...
def keyboard(key, x, y):
    ...
    global selected_tiles, riddle_active  # ADD THIS LINE
    ...
    elif key=='e':  # E key to select nearest tile (alternative to right-click)
        if riddle_active:
            select_nearest_tile()
    elif key.lower() == 'n':
        if dragon_healed:
...
def display():
    ...
    # Draw riddle UI with proper spacing
    if riddle_active:
        draw_riddle_tiles()
...
def mouse(button, state, x, y):
    ...
    # Print debug info
    button_names = {GLUT_LEFT_BUTTON: "LEFT", GLUT_RIGHT_BUTTON: "RIGHT", GLUT_MIDDLE_BUTTON: "MIDDLE"}
    state_names = {GLUT_DOWN: "DOWN", GLUT_UP: "UP"}
    print(f"Mouse: {button_names.get(button, str(button))} button {state_names.get(state, str(state))} at ({x}, {y})")
    
    if riddle_active and button == GLUT_RIGHT_BUTTON and state == GLUT_DOWN:
        print("Right-click detected during riddle!")
        ...
        for tile in riddle_tiles:
            ...
            if distance < 2.0:  # If player is close to tile
                ...
                tile["collected"] = False  # Make it collectible again
                print(f"Tile replaced with: {new_letter}")
